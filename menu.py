...
def Optimize():
    if settings.app.imported == True:
        ...
        if gui.budgetDropMenu.dropDownVar.get() == 'Select One' or gui.budgetDropMenu.dropDownVar.get() == 'Select status':
        ...
        #otherwise, just display the selected column (right now uses budgetDropMenu)
            ...
            displayColumn = pandaInput[gui.budgetDropMenu.dropDownVar.get()]
            label = tk.Label(gui.bot, width = 10, height = 2, \
                              text = gui.budgetDropMenu.dropDownVar.get(), relief = tk.RIDGE)
